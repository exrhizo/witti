...
extern "C"
{
	...
	void GLvelodyne::enableCartesian( bool trueOrFalse ) {
		computeCartesian = trueOrFalse;
	}

	void GLvelodyne::useCartesianFile( bool trueOrFalse ) {
	...
	void GLvelodyne::loadCalibration( const char *directory ) {
		loadCalibration(std::string(directory));
	}

	void GLvelodyne::loadCalibration( std::string directory ) {
		std::string dataFile;
	...
	int GLvelodyne::openFile( const char *filename ) {
		useFile = true;
	...
	int GLvelodyne::openAdapter( const char *adaptername ) {
		useFile = false;
		...
		if (pcapOpened) {
			...
			if((res = pcap_next_ex( fp, &header, &pkt_data)) > 0)
			{
				...
				if (computeCartesian) {
					...
					/*
					if((rot(0,0) < 45*MB_PI/180.0) || (rot(0,0) > 315*MB_PI/180.0))
					{
						for (int i = 0; i < 128; i++) {	// for all 128 laser measurements

							Dcorr = correctionValues(2, i);				// Distance correction factor
							Vo = correctionValues(3, i)/100;				// Vertical offset, in Z-direction
							Ho = correctionValues(4, i)/100;				// Horizontal offset, in X-direction
							...
							Vo += heightFromGround;	// account for height from ground
							//#define USE_MB_MATH
							// precompute:
#ifdef USE_MB_MATH
							totalOffset(0,0) = Ho;
							...
							for (int j = 0; j < FULL_MEASUREMENTS_PER_PACKET; j++) {	// for each of the three slices
								...
								double cp = cos(phi(j, i)), sp = sin(phi(j, i));
								Py(j, i) = dt(j, i) * cost*(sp*ca - cp*sa) - Ho*(cp*ca + sp*sa);
								Px(j, i) = dt(j, i) * cost*(cp*ca + sp*sa) - Ho*(sp*ca - cp*sa);
								Pz(j, i) = dt(j, i) * st + Vo;
					...
					*/
						...
						if (saveToCartesianFile && threadInfo[1].saveMe && 0) {
...
//#define USE_MB_MATH
	...
	{
		GLvelodyneThreadInfo *threadInfo = (GLvelodyneThreadInfo *)arg;
		double Dcorr;
		double Vo;
		double Ho;
		double alpha;
		...
		if(((*threadInfo->rot)(threadInfo->dataIndex,0) < (threadInfo->viewingAngle)*MB_PI/360.0) || ((*threadInfo->rot)(threadInfo->dataIndex,0) > (360-threadInfo->viewingAngle/2)*MB_PI/180.0))
		{
			...
			for (int i = 0; i < 128; i++) {	// for all 128 laser measurements
				...
				int j = threadInfo->dataIndex;
					...
					// Preprocessing of measured distances:
					if ((*threadInfo->dt)(j, i) < threadInfo->dtmin) {
						(*threadInfo->dt)(j, i) = 50000;
					}
					...
					// Now perform measurements:
					if ( ((*threadInfo->Pz)(j, i)  < threadInfo->zmin) ) {
						(*threadInfo->Px)(j, i) = 1000;
						(*threadInfo->Py)(j, i) = 1000;
						(*threadInfo->Pz)(j, i) = 1000;
					}
				}
		} else {
			threadInfo->saveMe = false;
		}
		pthread_exit(NULL);
